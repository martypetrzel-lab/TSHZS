  ...
  try {
    ...
    if (!user || !user.active || !(await compare(parsed.data.password, user.passwordHash))) {
      try {
        ...
        logServerError("Audit neúspěšného přihlášení se nepodařilo uložit.", auditError);
      }
    ...
    try {
      ...
      logServerError("Audit úspěšného přihlášení se nepodařilo uložit.", auditError);
    }
  } catch (error) {
    logServerError("Přihlášení selhalo kvůli chybě serveru.", error);
    return { error: "Přihlášení se nepodařilo kvůli chybě serveru. Zkuste to znovu nebo kontaktujte správce." };
  }
...
function logServerError(message: string, error: unknown) {
  const detail = error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : { type: typeof error };
