...
export function validateChecklistCompletion(answers: ChecklistAnswer[]) {
  const missing = answers.filter(
    (answer) =>
      answer.required && answer.value == null && !answer.notApplicable,
  );
  const invalidNa = answers.filter(
    (answer) =>
      answer.notApplicable &&
      (!answer.allowNotApplicable || !answer.reason?.trim()),
  );
  return {
    valid: missing.length === 0 && invalidNa.length === 0,
    missing,
    invalidNa,
  };
}
